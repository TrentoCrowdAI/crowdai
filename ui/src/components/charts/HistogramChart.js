...
class HistogramChart extends React.Component {
  ...
  buildGraph() {
    ...
    //scale to dynamically build intervals
    ...
    bar.append("rect")
      ...
      .on("click", (d) => {
        //on click build a new set of data with the object in the corresponding interval
        this.setState({
          clicked : []
        })
  ...
  render() {
    ...
    var x = this.props.x
    var z = this.props.z
    //details of clicked data
    var stampa = this.props.data.length>0 ? this.state.clicked.map(d => 
      <li key={d[x]+""+d[y]}>
        {y+" "}<strong style={{color: '#2185d0'}}>{d[y]}</strong>
        {", "+z+" "}<strong style={{color: '#2185d0'}}>{d[z]}</strong>
        {" => "+x+" "}<strong style={{color: '#2185d0'}}>{d[x]}</strong>
      </li>) : " "
    ...
    return (
      <div>
        ...
        { this.props.data.length &&
          ...
          //display a new chart with elements in the clicked interval
            <ChartWrapper 
              color={'orange'}
              x={this.props.y}
              y={this.props.x}
