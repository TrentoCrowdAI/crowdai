...
const { JobStatus, JobEstimationStatus } = require(__base + 'utils/constants');
...
/**
 * Calls the POST /estimates endpoint of the estimations-api.
 *
 * @param {Object} jobId
 * @param {Object} single - True means only run a single estimation based on all the parameter of the job. False means
 *                          run estimations base on multiple values of votesPerTaskRule and initialTestsRule
 * @return {Object} The token generated by the estimates endpoint. The return value looks like the following:
 *
 * @example
 * {token: '5cc8afa4-2e42-42d8-ae3d-dd2d35e07347'}
 */
exports.computeEstimates = async (jobId, single) => {
  if (!jobId) {
    throw Boom.badRequest('The jobId is required');
  }
  let job = await delegates.jobs.getById(jobId);
  ...
  if (payload.baseroundItems % payload.itemsPerWorker !== 0) {
    const d = Math.floor(payload.baseroundItems / payload.itemsPerWorker);
    payload.baseroundItems = (d + 1) * payload.itemsPerWorker;
  }
  ...
  try {
    let response = await new Promise((resolve, reject) => {
      request(
        {
          url: `${config.estimations.url}/estimates`,
          method: 'POST',
          json: {
            ...payload
          }
    ...
    job.data.estimationToken = response.token;
    await delegates.jobs.update(job.id, job);
    return response;
  } catch (error) {
    throw Boom.badImplementation(
      'Error while running job estimations endpoint'
    );
  }
};

/**
 * Calls the GET /status/<token> endpoint of the estimations-api. When this endpoint
 * returns DONE, we fetch the estimations and store as a property of the job.
 *
 * @param {Object} jobId
 * @return {Object} The status returned by the estimations-api endpoint:
 *
 * @example
 * {status: DONE | IN_PROGRESS | NONE}
 */
exports.checkEstimatesStatus = async jobId => {
  if (!jobId) {
    throw Boom.badRequest('The jobId is required');
  }
  let job = await delegates.jobs.getById(jobId);
  ...
  try {
    let statusRsp = await new Promise((resolve, reject) => {
      request(
        {
          url: `${config.estimations.url}/status/${job.data.estimationToken}`,
          method: 'GET'
        },
        (err, rsp) => {
          if (err) {
    ...
    if (
      statusRsp.status === JobEstimationStatus.IN_PROGRESS ||
      statusRsp.status === JobEstimationStatus.NONE
    ) {
      return statusRsp;
    }
    ...
    job.data.estimations = rsp;
    await delegates.jobs.update(job.id, job);
    return statusRsp;
  } catch (error) {
    throw Boom.badImplementation(
      'Error while checking the status of the estimations'
    );
  }
};
